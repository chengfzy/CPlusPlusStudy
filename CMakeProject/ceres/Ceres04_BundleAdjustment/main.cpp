...
class BALProblem {
   public:
    ...
    double* mutablePoints() {
        return parameters_ + 9 * numCameras_;
    }

    double* mutableCameraForObservation(size_t i) {
        return mutableCameras() + cameraIndex_[i] * 9;
    }
    double* mutablePointForObservation(size_t i) {
...
struct SnavelyReprojectionError {
    SnavelyReprojectionError(double observedX, double observedY) : observedX(observedX), observedY(observedY) {
    }
    ...
    bool operator()(const T* const camera, const T* const point, T* residuals) const {
        ...
        // compute the center of distoration, the sign change comes from the camera model that Noah Snavely's Bundler
...
int main(int argc, char** argv) {
    ...
    if (!balProblem.loadFile("../../Ceres04_BundleAdjustment/problem-49-7776-pre.txt")) {
    ...
    for (size_t i = 0; i < static_cast<size_t>(balProblem.numObservations()); ++i) {
        // Each residual block takes a point and a camera as input and outputs a dimensinal residual. Internally, the
    ...
    // Make Ceres automatically detect the bundle structure. Note that the standard solver, s SPARSE_NORMAL_CHOLESKY,
